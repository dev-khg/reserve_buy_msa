...
import hg.reserve_buy.orderserviceapi.infrastructure.redis.RedisExecutorProxy;
...
public class StockCacheService {
    private final RedisExecutorProxy redisExecutorProxy;

    public void reserveStock(String orderId, Long itemNumber, Integer count) {
        String key = RedisKey.REDIS_STOCK_PREFIX + itemNumber;

        Boolean success
                = redisExecutorProxy.executeTemplate(RedisTimeDealScripts.reserveOrderScript, List.of(key), count);
        ...
        redisExecutorProxy.putValue(reserveKey, null, 2, TimeUnit.MINUTES);
    }
